...
export default class SearchCourse extends Component {
	...
	//初始化数据（讲师、一级课程分类）
	initData = async()=>{
		/* let no1SubjectList =	await reqAllNo1Subject()
		let teachers =	await reqGetAllTeacherList() */
			...
			<Card>
				...
				>
					<Item label="标题" name="title">
						<Input placeholder="输入标题" className="search_title"/>
					</Item>
					...
					<Item>
						...
						<Button onClick={()=>this.form.resetFields()} className="search_reset">重置</Button>
					</Item>
				</Form>
			</Card>
		)
